...
const saved = localStorage.getItem('loadgame');
if (saved !== null) {
  const parser = new DOMParser();
  const doc = parser.parseFromString('<div>' + saved + '</div>', 'text/html');
  const src = doc.body.firstChild;
  const unsafe = src.querySelector('script') ||
    [...src.querySelectorAll('*')].some(el =>
      [...el.attributes].some(a => /^on/i.test(a.name)));
  if (unsafe) {
    localStorage.removeItem('loadgame');
    generate();
  } else {
    while (src.firstChild) loadgame.appendChild(src.firstChild);
...
function check() {
  ...
  if (correctCells.length == numbersize * 2) {
    Array.from(document.querySelectorAll('.number:not(.delete)')).forEach((cell) => cell.classList.add('circle'));
